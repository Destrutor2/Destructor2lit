				...
				if (entity.getCustomName() != null) {
					if (entity.getCustomName().equalsIgnoreCase(player.getDisplayName()) && entity instanceof Zombie) {
						...
						if (main.hasBed(utils.getMetadata(entity, "color").asString()) || utils.getMetadata(entity, "alive").asBoolean()) {
							...
							entity.remove();
							player.sendMessage(ChatColor.RED + "You lost your bed and died while you were disconnected");
						}
					}
				}
			}
...
//        ###################IMPLEMENTER LES COULEURES DE RANK DES QUE POSSIBLE ###################################
				...
				if (main.getPlayers().size() == 1) {
					for (Player p : Bukkit.getOnlinePlayers()) {
						p.sendMessage(ChatColor.RED + "We don't have enough players! Start cancelled.");
					}
				}
			} else {
	...
	public void onBlockPlace(BlockPlaceEvent e) {
		...
		if (!new Utils().getMetadata(player, "alive").asBoolean() || e.getBlock().getY() >= main.buildlimit) {
			e.setCancelled(true);
	...
	public void onEntityDamagedByEntity(EntityDamageByEntityEvent e) {
		...
		if (damager instanceof IronGolem) {
			e.setDamage(e.getDamage() * main.golemDamageMultiplier);
		}
...
//		On ne veut pas que les zombies des joueurs afk attaquent d'autres joueurs mais on veut quand meme qu'ils les regardent
		...
		if (damager instanceof Player) {
			...
			if (utils.getMetadata(damager, "invincible").asBoolean() && damager != entity) {
...
//		Cas de kill:
	...
	void onEntityDamage(EntityDamageEvent e) {
		...
		if (e.getCause().equals(EntityDamageEvent.DamageCause.VOID)) {
			e.setCancelled(true);
//			Die.Void(player, main.getPlayers(), main);
			return;
		}
		if (e.getEntity() instanceof Player) {
			if (e.getCause().equals(EntityDamageEvent.DamageCause.FALL) && e.getFinalDamage() >= ((Player) e.getEntity()).getHealth()) {
				e.setCancelled(true);
				Die.Fall(player, main);
			}
		}
	}
	...
	public void onPlayerDie(PlayerDeathEvent e) {
		Bukkit.broadcastMessage(ChatColor.RED + "Oops le type de mort de " + e.getEntity().getDisplayName() + " n'a pas encore été implémenté :o");
	...
	public void onTeleport(PlayerTeleportEvent e) {
		if (e.getCause().equals(PlayerTeleportEvent.TeleportCause.ENDER_PEARL)) {
			...
			if (!illegaltp) {
				...
				e.getPlayer().sendMessage(ChatColor.RED + "Tu ne peux pas te tp sur un npc!");
			}
		}
	}
...
//		Le truc pour si on tombe dans le void:
		if (e.getTo().getY() < main.getConfig().getDouble("voidheight")) {
			for (Entity entity : e.getPlayer().getWorld().getEntitiesByClasses(EnderPearl.class))
				if (((EnderPearl) entity).getShooter() == e.getPlayer())
//					de cette manière, si le joueur a lancé une pearl, on attends un peu avant de le faire tomber dans le void
					if (entity.getLocation().getY() > main.voidHeight)
						return;


			Die.Void(e.getPlayer(), main);
		}
...
//			Tant qu'on est la, si le joueur s'approche a plus de 30 blocks du npc, il va se faire respawn
	...
	@EventHandler
	public void onBlockPlaceIgnoreCancelled(BlockCanBuildEvent e) {
		e.setBuildable(true);
	}
	...
	public void onBowShoot(EntityShootBowEvent e) {
		if (e.getForce() < main.minBowCharge && e.getEntity() instanceof Player) {
			e.setCancelled(true);
